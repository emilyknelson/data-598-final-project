...
```{r modeling1b}
ar2_ma5 = sarima(gdp_log, p = 2, d = 1, q = 3)
```
...
** ARIMA Modeling ** 
```{r modeling3a}
ar4_ma4 = sarima(debt_log, p = 4, d = 2, q = 2)
```
...
This is an interesting series - it's definitely non-stationary, but not in a constant way. There is a period of time where international freight traffic really fell from 2002 through 2014. 
...
```{r seasonality}
flights_spec = mvspec(flights, spans = 2) # No need for detrend because there's not a strong trend
```
...
## Additional Analysis
For additional analyses, I was hoping to do three things. 
